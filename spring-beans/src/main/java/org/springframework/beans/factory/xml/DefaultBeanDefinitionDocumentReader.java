...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
		...
		if (delegate.isDefaultNamespace(root)) {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					//其他命名空间,如<context:component-scan/>标签的命名空间http://www.springframework.org/schema/context
					else {
	 ...
	 * 关键方法
	 * 解析<bean>元素,并注册
	 * 1.解析bean元素,bean元素中的id,class等属性,放入beanDefinition
	 * 2.注册,就是把bean放入beanFactory中的beanDefinitionMap中
	 */
