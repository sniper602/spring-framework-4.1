...
/**
 * BeanFactory实现抽象基类，提供可配置BeanFactory SPI的全部功能。
 * 不假定是可列表的bean工厂:因此也可以用作bean工厂实现的基类，
 * 这些实现从一些后端资源获取bean定义(其中bean定义访问是一个昂贵的操作)。
 该类提供了一个单例缓存(通过它的基类DefaultSingletonBeanRegistry、单例/原型确定、
 FactoryBean处理、别名、用于子bean定义的bean定义合并和bean销毁(可丢弃bean接口、自定义销毁方法)。
 此外，它还可以通过实现HierarchicalBeanFactory接口来管理bean工厂层次结构(在未知bean的情况下委托给父bean)。

 子类实现的主要模板方法是getBeanDefinition(java.lang. string)和createBean(java.lang. string)。
 字符串,org.springframework.beans.factory.support。分别检索给定bean名称的bean定义和为给定bean定义创建bean实例。
 这些操作的默认实现可以在DefaultListableBeanFactory和abstractautowiablebeanfactory中找到。
