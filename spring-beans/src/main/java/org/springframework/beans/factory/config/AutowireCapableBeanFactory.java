...
/**
 * bean工厂实现的BeanFactory接口的扩展，如果它们希望为现有的bean实例公开此功能，则这些工厂能够自动连接。
 BeanFactory的这个子接口不打算在常规应用程序代码中使用:对于典型用例，请坚持使用BeanFactory或ListableBeanFactory。
 其他框架的集成代码可以利用这个接口连接和填充Spring不控制其生命周期的现有bean实例。例如，这对于WebWork操作和Tapestry页面对象特别有用。
 注意，这个接口不是由ApplicationContext facade实现的，因为应用程序代码很少使用它。也就是说，它也可以从应用程序上下文中获得，
 可以通过ApplicationContext的ApplicationContext. getautowiablebeanfactory()方法访问。
 您还可以实现BeanFactoryAware接口，该接口公开内部BeanFactory，即使在ApplicationContext中运行时，
 也可以访问AutowireCapableBeanFactory:只需将传入的BeanFactory转换为AutowireCapableBeanFactory。
