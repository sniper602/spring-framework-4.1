...
/**
 * 共享bean实例的通用注册表，实现了SingletonBeanRegistry。允许注册应该为注册表的所有调用者共享的单例实例
 * ，这些单例实例将通过bean名称获得。还支持在注册表关闭时销毁可丢弃bean实例(这些实例可能对应于已注册的单例)。
 * 可以注册bean之间的依赖关系来强制执行适当的关闭顺序。

 这个类主要用作BeanFactory实现的基类，分解出单例bean实例的公共管理。
 注意，ConfigurableBeanFactory接口扩展了SingletonBeanRegistry接口。

 注意，与AbstractBeanFactory和DefaultListableBeanFactory(继承自它)相比，该类既不假定bean定义概念，
 也不假定bean实例的特定创建过程。也可以作为一个嵌套的助手来委托。
