 ...
 * @since 16.03.2003
 *
 * 这个类可用于解析公共静态最终成员中包含常量定义的其他类。该类的{@code asXXXX}方法允许通过字符串名访问这些常量值。
 *考虑类Foo包含{@code public final static int CONSTANT1 = 66;}
 *这个类的一个实例包装{@code Foo。类}将从其{@code asNumber}方法返回66的常量值，前提是参数{@code "CONSTANT1"}。
 *这个类非常适合在PropertyEditors中使用，使它们能够识别与常量本身相同的名称，并将它们从维护自己的映射中解放出来。
 */
